1. 为统一支持阿里云/阿里云国际，修改bss接口地址business.aliyuncs.com

2. 更改BSS账单所调用的接口为DescribeInstanceBill，且账单金额仅统计配置的实例ID，EIP在绑状态不会产生账单，而流量部分只需关注流量统计故也不计算金额
...
要真正实现“止损脚本只能控制这一台机器”，你需要按照以下步骤操作：

### 步骤 1：在阿里云后台创建/确认资源组

1. 登录阿里云控制台，搜索 **“资源组”**。
2. 确认你的 ECS 实例已经移动到了某个特定的资源组（默认通常在“默认资源组”）。
3. 记录下该资源组的 **ID**（通常以 `rg-` 开头）。

### 步骤 2：进行 RAM 授权（关键）

**不要**在 RAM 用户的“个人详情 -> 权限管理”页面直接添加权限（那是全局权限）。
...
3. 点击 **“新增授权”**：
   ...
   - **被授权主体**：选择你为脚本创建的 RAM 用户。
   - **角色策略**：搜索并添加 `AliyunECSFullAccess`。

   > **注意**：这样授权后，该 RAM 用户**仅对该资源组内**的 ECS 有开关机权限，账号下其他资源组的机器它看不见也动不了。

### 步骤 3：保留账号级只读权限

由于账单（BSS）和流量统计（CDT）是**账号维度**的资源，无法收敛到资源组。你仍需在 **RAM 用户控制台**（全局权限）为其添加：
